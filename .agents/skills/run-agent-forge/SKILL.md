...
Agent Forge es un servidor Express (`server/`) que orquesta ocho fases del Codex Agent SDK
...
explorador de archivos— **sin gastar un solo token**. Solo el modo `live` llama a Codex.
...
- **Credenciales solo para `live`.** Con `ANTHROPIC_API_KEY` en `server/.env` se factura por
  token; sin ella, el SDK usa la sesión de Codex y consume cuota de suscripción.
...
npm install
npm run install:all
npm --prefix .Codex/skills/run-agent-forge install
```
...
El harness vive en `.Codex/skills/run-agent-forge/driver.mjs`. Arranca el servidor, espera a
que escuche, afirma, y **mata el árbol de procesos al terminar**.

### 1. Smoke: API y replay, sin cuota

```bash
node .Codex/skills/run-agent-forge/driver.mjs smoke
```
...
```
  ...
  ✓ servidor arriba en http://localhost:5178 (modelo Codex-opus-5)
·   aviso: sin ANTHROPIC_API_KEY -> usara las credenciales de Codex
...
```bash
node .Codex/skills/run-agent-forge/driver.mjs ui
```
...
UI OK — capturas en .Codex\skills\run-agent-forge\shots/
```

Deja cuatro PNG en `.Codex/skills/run-agent-forge/shots/`:
...
```bash
node .Codex/skills/run-agent-forge/driver.mjs live
```
...
`node .Codex/skills/run-agent-forge/driver.mjs live "Un lector de RSS con carpetas y modo lectura"`.
...
```bash
node .Codex/skills/run-agent-forge/driver.mjs serve
```

## Trampas

- **`allowedTools` anula el guard.** Poner un nombre de herramienta ahí hace que el SDK la
  autoapruebe _antes_ de llamar a `canUseTool`, y la contención de rutas de `server/src/guard.ts`
...
- **El servidor se reutiliza.** Si ya hay algo escuchando en 5178, el driver lo usa en vez de
  arrancar el suyo — y entonces _no_ lo mata al salir. Cómodo, pero si ese proceso es viejo
