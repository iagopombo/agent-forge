...
function ensureWebBuilt() {
  ...
  const r = spawnSync("npm --prefix web run build", {
    cwd: UNIT,
    stdio: "inherit",
    shell: true,
  });
...
async function startServer() {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (serverProc.exitCode !== null)
      fail(`el servidor murio al arrancar:\n${out}`);
    await sleep(500);
  }
...
async function readEvents(
  runId,
  { seconds = 20, until = null, from = null } = {}
) {
  ...
  try {
    const res = await fetch(`${BASE}/api/runs/${runId}/events${query}`, {
...
function installFixture() {
  const raw = fs.readFileSync(
    path.join(SKILL_DIR, "fixture", "events.jsonl"),
    "utf8"
  );
...
async function smoke() {
  ...
  check(
    roles.length === 8,
    `GET /api/roles devuelve las 8 fases (${roles.map((r) => r.short).join(", ")})`
  );
  ...
  check(
    files.length > 0,
    `GET /files lista ${files.length} archivos del workspace`
  );
  ...
  check(
    events[events.length - 1].t === "run.end",
  ...
  const travesias = [
    [
      `/api/runs/${FIXTURE_ID}/file?path=../../../server/.env`,
      "ruta que sale del workspace",
    ],
    [
      `/api/runs/${FIXTURE_ID}/file?path=..%2f..%2f..%2fserver%2f.env`,
      "ruta escapada en URL",
    ],
    [
      `/api/runs/${FIXTURE_ID}/file?path=C:\\Windows\\win.ini`,
      "ruta absoluta del sistema",
    ],
    [
      `/api/runs/${FIXTURE_ID}/file?path=docs`,
      "un directorio en vez de un archivo",
    ],
  ...
  check(
    spa.status === 200 && (await spa.text()).includes('<div id="root"'),
    "cualquier ruta sirve la SPA"
  );
  check(
    (await fetch(`${BASE}/api/no-existe`)).status === 404,
...
async function ui() {
  ...
  try {
    ...
    check(
      (await page.inputValue(".field textarea")).length > 40,
      "el ejemplo rellena la idea"
    );
    check(
      await page.isEnabled(".primary"),
    ...
    await page.waitForSelector(
      ".transcript-body .say, .transcript-body .think",
      { timeout: 20000 }
    );
    ...
    check(
      (await page.textContent(".code")).length > 100,
    ...
    check(
      errors.length === 0,
      `sin errores de consola${errors.length ? `: ${errors[0]}` : ""}`
    );
  } finally {
    await browser.close();
  }
...
async function live(idea) {
  ...
  const prompt =
    idea ??
    "Un panel para que un fisioterapeuta gestione pacientes, sesiones y bonos.";
  ...
  check(
    wrote.length > 0,
    `el agente pidio escribir ${wrote.map((e) => e.path).join(", ")}`
  );
  ...
  check(
    onDisk.length > 0,
    `el workspace tiene ${onDisk.length} archivo(s) en disco`
  );
  const doc = await api(
    `/api/runs/${run.id}/file?path=${encodeURIComponent(onDisk[0].path)}`
  );
  check(
    doc.content.length > 500,
    `${onDisk[0].path} trae ${doc.content.length} caracteres reales`
  );
