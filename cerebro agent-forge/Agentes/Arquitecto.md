...
Su prompt de asesor distingue dos tipos de consulta: una decisión de
arquitectura real (hueco, ambigüedad, contradicción) — respuesta corta y
decidida, sin alternativas abiertas, y si cambia el contrato ya documentado lo
dice explícitamente y señala qué documento actualizar — y una reconfirmación
de un dato exacto que ya existe en disco (el ingeniero lo vio pero perdió el
detalle, típicamente tras una compactación de contexto) — ahí solo cita el
dato, sin la ceremonia de justificar una decisión. Este segundo caso se añadió
tras [[Kairos, bot de trading (prueba)]]: releer un documento entero para
recuperar un solo dato es lo que dispara el bucle de recompactación que puede
abortar la fase. No edita: solo aconseja, el ingeniero aplica el cambio.
