...
`server/src/ask.ts`. La tool `preguntar_al_usuario` en sí vive como
`askUserTool()`, exportada aparte de `buildAskServer()` porque más de una
fase la necesita: [[Producto]] (el único con `canAsk: true` en `roles.ts`) la
registra sola vía `buildAskServer`; [[Diseño]] (`canDesign: true`) la
reutiliza tal cual, combinada con su propia tool `mostrar_diseno`, en el
mismo servidor MCP `forge` que construye `buildDesignServer()` en
`server/src/design.ts` — cada fase solo puede registrar un servidor `forge`,
así que las dos tools de diseño viven juntas ahí.

> [!info] Dos usos del mismo mecanismo en [[Diseño]], no confundir
> `preguntar_al_usuario` (sin imagen): la entrevista de estilo, una vez, al
> principio de la fase, antes de tocar el sistema de diseño.
> `mostrar_diseno` (con imagen, `Run.askUser(phase, ..., image)`): la
> iteración pantalla a pantalla, en bucle, después. Las dos bloquean la fase
> igual y comparten el mismo `Run.askUser()` de fondo. Ver
...
que de verdad _bifurcan_ el producto: a qué público si caben distintos, qué
