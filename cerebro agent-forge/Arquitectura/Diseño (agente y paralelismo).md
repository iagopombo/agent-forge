...
## También entrevista con `preguntar_al_usuario`, antes de maquetar

`buildDesignServer()` no registra solo `mostrar_diseno`: también incluye
`askUserTool()` (la misma tool que [[Producto]], extraída de
`server/src/ask.ts`), porque cada fase solo puede tener un servidor MCP
`forge`. El agente la usa una vez, al principio, para fijar decisiones de
estilo que el brief deja abiertas (tono, referencias, tema) — sin imagen, 3
opciones, igual que produce. Después sigue con `mostrar_diseno` en bucle como
se describe arriba. Detalle completo en [[Diseño]] y
