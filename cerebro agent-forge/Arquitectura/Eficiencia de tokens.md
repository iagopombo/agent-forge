...
> [!bug] No reconstruir tras perder detalle (p. ej. tras compactar)
> Añadido tras [[Kairos, bot de trading (prueba)]]: relee un documento entero
> para recuperar un solo dato perdido (típicamente justo después de que
> Claude Code compacte el contexto) es el error más caro posible — vuelve a
> llenar el contexto casi de inmediato y puede disparar el bucle de
> recompactación que aborta la fase entera ("autocompact is thrashing"). La
> Skill ahora lo dice explícitamente: confiar en el resumen para lo ya
> decidido, y para el dato puntual usar Grep o preguntar al
> `architect-advisor` (corre en su propio contexto — el documento que
> consulte no vuelve a entrar en el del que preguntó).
>
> El `architect-advisor` (`ARCHITECT_ADVISOR_PROMPT`, `roles.ts`) distingue
> ahora dos tipos de consulta con dos respuestas distintas: una decisión de
> arquitectura real (hueco, ambigüedad, contradicción — justifica y decide) y
> una reconfirmación de un dato exacto que ya existe en disco (solo cita el
> dato, sin la ceremonia de justificar nada).
>
> Se descartó un hook `PostCompact` para esto: `systemMessage`/
> `additionalContext` en ese hook son solo para la interfaz, el modelo nunca
> los ve, y no hay ningún otro hook entre una compactación y el siguiente
> turno del modelo que sirva en una fase no interactiva (una sola llamada a
> `query()` por fase, sin "siguiente prompt del usuario" a mitad). Detalle
> completo en [[Kairos, bot de trading (prueba)]].
