...
| `workspace`   | Producto decidió el nombre y la carpeta se renombró (`renameToProductName`) | `workspace`                                       |
...
| `ask`         | [[Producto                                                                  | producto]] o [[Diseño                             | diseño]] preguntan al usuario (`image` si es diseño mostrando una captura) | `phase`, `id`, `question`, `options`, `image?` |
...
(`phase.end`, `run.end`, `run.start`, `workspace`) se reescribe el manifiesto
`.runs/<id>.json` — ver [[Workspaces y slugs]]. `workspace` se sumó a esa
lista junto con el evento: sin él, un rename de carpeta a mitad de fase no
quedaba en disco hasta el siguiente `phase.end`, y una caída del servidor
antes de eso dejaba el manifiesto apuntando a una carpeta ya inexistente —
ver [[Workspaces y slugs#El problema que esto le crea a reanudar, y cómo se resolvió]].

> [!bug] `writeManifest()` encadenado por id, no suelto — tumbaba el servidor entero
> Diseño y arquitecto sembrados de golpe al retomar disparan varios
> `phase.end` casi en el mismo tick, cada uno con `void this.writeManifest(run)`
> sin `.catch()`. Dos `rename()` casi simultáneos sobre el mismo
> `.runs/<id>.json` pueden pisarse en Windows (`EPERM`), y una promesa
> rechazada sin capturar es una excepción no controlada que mata el proceso
> — no solo la ejecución, el servidor entero. Arreglo: `RunStore` encola los
> `writeManifest()` de cada id (`manifestQueues`) y traga cualquier fallo de
> escritura en vez de propagarlo. Detalle completo, con la cadena de
> síntomas real que llevó hasta aquí, en
> [[Fiabilidad del servidor de fondo#`writeManifest()` sin encolar tumbaba el servidor al retomar]].
