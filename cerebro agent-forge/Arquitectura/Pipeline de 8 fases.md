...
> donde sí se asume `pass: true` es cuando la fase _terminó bien_ pero
...
fase con una nota en el contexto: _"Retomas una ejecución interrumpida (...)
lee docs/ y el código existente antes de escribir nada"_.

> [!bug] Ese aviso vivió roto (silenciosamente) hasta [[Kairos, bot de trading (prueba)]]
> El aviso se guardaba en `ctx.previous` — el mismo campo que cada fase
> anterior (producto, backend...) sobrescribe con su propio resumen antes de
> que le toque el turno a la fase realmente retomada. Nunca llegaba. Arreglado
> con un campo aparte, `ctx.resumeNote`, entregado una sola vez y solo a la
> fase exacta que se retoma — detalle mecánico en
> [[Orquestador#Aviso de reanudación (resumeNoteFor)]].
