...
El slug derivado de la idea es el nombre de carpeta _inicial_, no el
...
`RunStore.resolveWorkspace(slug)` (público, ya no `private` — `index.ts`
también lo usa, ver más abajo) mira todos los manifiestos que comparten slug
(mismo patrón que `priorPhaseOutcomes`) y devuelve el `workspace` del más
reciente; `create()` lo usa cuando la petición trae `startFrom` y no
especifica `workspace` explícito. Así una reanudación después de un rename
sigue encontrando la carpeta correcta.

> [!bug] El manifiesto podía quedarse con la ruta vieja igualmente — encontrado con una ejecución real (Kairos)
> `resolveWorkspace()` lee `workspace` de `.runs/<id>.json`, y ese fichero
> solo se reescribe (`RunStore.writeManifest`) al ver `phase.end`, `run.end` o
> `run.start`. `renameToProductName()` reasignaba `this.workspace` pero solo
> emitía un evento `log` — si el servidor moría antes del siguiente de esos
> tres eventos (p. ej. diseño y arquitecto seguían sin terminar cuando
> `node --watch` reinició el proceso tras una edición de código), el
> manifiesto se quedaba para siempre con la carpeta pre-rename, que ya no
> existe. Encima, `POST /api/runs` en `index.ts` reconstruía la ruta a mano
> con `path.join(workspacesRoot, projectSlug(idea))` en vez de llamar a
> `resolveWorkspace` — doble fallo. Síntoma real: "Retomar" devolvía 400 "No
> existe el workspace de la ejecución que quieres retomar." con el workspace
> existiendo de verdad, solo que con otro nombre.
>
> Arreglo: nuevo evento `{ t: 'workspace', workspace }` (`events.ts`), que
> `renameToProductName()` emite justo tras reasignar `this.workspace` (antes
> del `log` de aviso) y que `RunStore` añadió a su lista de eventos que
> disparan `writeManifest()` — el manifiesto queda correcto en disco al
> instante del rename, no en el próximo `phase.end`. `index.ts` pasó a usar
> `runs.resolveWorkspace(slug) ?? path.join(workspacesRoot, slug)` para la
> comprobación de existencia, en vez de reconstruir la ruta por su cuenta.
> Verificado en real: antes del arreglo, `POST /api/runs` con `resumeOf`
> devolvía 400; después, 201 con `workspace: "...\\kairos"`, y la ejecución
> retomada sembró `product` como ya hecho y relanzó `design`+`architect` en
> paralelo. Los dos manifiestos ya corrompidos de ese proyecto
> (`quiero-hacer-un-bot-de-5861fa.json` y `-7310ac.json`) se repararon a
> mano. Detalle completo en [[Fiabilidad del servidor de fondo]].
...
El id identifica la _ejecución_ (para el registro de eventos, para
`/api/runs/:id/...`); el slug identifica el _proyecto_ (para la carpeta y la
...
aquí (después de que `workspaceFor(id)` valide la _forma_ del id — ver
