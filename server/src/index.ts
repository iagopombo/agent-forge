...
app.get("/api/health", (_req, res) => {
  res.json({ ok: true, model: CONFIG.model, engine: "opencode" });
});
...
app.post("/api/runs", (req, res) => {
  ...
  if (idea.length < 15) {
    res
      .status(400)
      .json({ error: "Describe la idea con al menos 15 caracteres." });
    return;
  }

  // Retomar: la misma idea da el mismo slug y por tanto el mismo proyecto. La
  // carpeta puede haberse renombrado a mitad del intento anterior (Producto
  // decide el nombre real, ver `Run.renameToProductName`), así que hay que
  // resolverla igual que hace `RunStore.create()` — reconstruirla a mano del
  // slug apuntaría a una carpeta que ya no existe.
  let startFrom: PhaseId | undefined;
  if (req.body?.resumeOf !== undefined) {
    const slug = projectSlug(idea);
    const workspace =
  ...
  const run = runs.create({
    ...
    effortCap: (EFFORTS as readonly string[]).includes(
      String(req.body?.effortCap)
    )
...
app.post("/api/runs/:id/answer", (req, res) => {
  ...
  if (!id || !answer) {
    res
      .status(400)
      .json({ error: "Faltan el id de la pregunta o la respuesta." });
    return;
  }
...
app.get("/api/runs/:id/events", async (req, res) => {
  ...
  if (!run) {
    ...
    if (valid)
      for (const event of await runs.replay(req.params.id, from))
...
app.get("/api/runs/:id/files", async (req, res) => {
  ...
  const workspace = workspaceFor(req.params.id)
    ? await runs.workspaceOf(req.params.id)
    : null;
...
app.listen(CONFIG.port, () => {
  ...
  console.log(`  Motor: OpenCode`);
  console.log(`  Modelo: ${CONFIG.model ?? "(por defecto por rol)"}`);
