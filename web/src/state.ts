...
export function applyEvents(state: RunState, events: ForgeEvent[]): RunState {
  ...
  let { runId, idea, workspace, status, costUsd, startedAt, endedAt, lastSeq } =
    state;
  let logs = state.logs;
  ...
  for (const event of events) {
    ...
    switch (event.t) {
      ...
      case "run.end": {
        ...
        if (event.status !== "done") {
          for (const id of PHASE_ORDER) {
            if (
              phases[id].status === "running" ||
              phases[id].status === "paused"
            ) {
      ...
      case "answer": {
        ...
        const phase = touch(event.phase);
        const block = [...phase.blocks]
          .reverse()
          .find((b) => b.kind === "ask" && !b.answer);
      ...
      case "paused": {
        ...
        const algunaCorriendo = Object.values(phases).some(
          (p) => p.status === "running"
        );
...
export function activePhase(state: RunState): PhaseId {
  const running = PHASE_ORDER.find(
    (id) => state.phases[id].status === "running"
  );
  if (running) return running;
  const touched = PHASE_ORDER.filter(
    (id) => state.phases[id].blocks.length > 0
  );
